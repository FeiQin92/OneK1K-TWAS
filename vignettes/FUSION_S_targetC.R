...
#               help="Force specific predictive model to be used, no flag (default) means select most significant cross-val. Options: targetC, S_targetC, S_allC"),
...
FUNSION <- function(opt){

	#opt$chr = c(1:22)[as.numeric(ID_for_chr)]
	#opt$weights = paste0(dir, "BC_fusion/weights/" ,tiss_name_ID, ".pos")
	#opt$out = paste0(dir, cancer_type, "_fusion/output_full_no_inflat/", tiss_name_ID, "_", ID_for_chr, "_output")

    print(paste0("Analysis of chr: ", opt$chr))
    print(paste0("Analysis of Tissue: ", tiss_name_ID))
   ...
   if (substr(genos$bim[1,2],1,2)!="rs") {genos$bim[,2] <- paste0(genos$bim[,1], "_", genos$bim[,4])}
...
ID_for_chr=args[1]
ID_for_tiss=args[2]
tiss_name_ID = c("CD4_ET","NK","CD4_NC","CD8_S100B","CD8_ET","B_IN","CD8_NC","B_Mem","NK_R","Mono_NC","Mono_C","DC","Plasma","CD4_SOX4")[as.numeric(ID_for_tiss)]
cancer_type="Melanoma"
dir <- c("/gpfs/gsfs12/users/qinf2/OneK1K/TWAS/logCPM/model/GWAS/")
FUNSION(opt=data.frame(sumstats=paste0("/gpfs/gsfs12/users/qinf2/OneK1K/TWAS/logCPM/model/GWAS/", cancer_type, "_fusion/Melanoma_fusion.txt"), 
                       out=paste0(dir, cancer_type, "_fusion/output_S_targetC/", tiss_name_ID, "_", ID_for_chr, "_output"), 
                       weights=paste0(dir, "BC_fusion/weights_org/" ,tiss_name_ID, ".pos"), 
                       weights_dir=paste0(dir, "BC_fusion/weights_org"), 
                       ref_ld_chr="/gpfs/gsfs12/users/qinf2/OneK1K/TWAS/logCPM/1000genomes/hg19/EUR/", 
                       force_model="S_targetC", 
                       caviar=FALSE, jlim=FALSE, 
                       max_impute=0.99, min_r2pred=0.1, 
                       perm=0, perm_minp=0.05, 
                       chr=c(1:22)[as.numeric(ID_for_chr)], 
                       coloc_P=NA, GWASN=NA, PANELN=NA))
